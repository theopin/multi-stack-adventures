...
export default class AccountService {
  ...
  static async authenticateAccount(username, password) {
    ...
    return {
      sessionToken: createJwtToken(matchingUser._id.toString(), 'user', process.env.JWT_SESSION_TOKEN_EXPIRY),
      refreshToken: createJwtToken(matchingUser._id.toString(), 'user', process.env.JWT_REFRESH_TOKEN_EXPIRY)
    };
  }
  ...
  static async getAccountById(token, id, username) {
  ...
  static async updateAccountById(token, id, balance, password) {
    const tokenDetails = await analyseJwtToken(token, id)

    return Helper.updateOne({ _id: id, balance: balanceCheck}, {$inc : {balance : change}}, {new: true})
  }

  static async deleteAccountById(token, id, username) {
    const tokenDetails = await analyseJwtToken(token, id)
